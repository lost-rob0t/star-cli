...
when isMainModule:
  const targetHelp = {"target": "target value or path to file when in bulk mode", "dataset": "Dataset for actors to store data in (actor is a bot)",
                      "actor": "name of the actor (aka a bot name)", "bulk": "Create many targets listed in the file set by target argument",
                     "options": "optional json object holding config for the actor. See the actor's documentation for what to put in here."}.toTable
  const importHelp = {"configPath": "path to config", "file": "path to a starintel document dump"}.toTable
  ...
  dispatchMulti([target, help=targetHelp], [init], [insert_data], [publishDocuments, help=publishHelp], [subscribe, help=subscribeHelp])
